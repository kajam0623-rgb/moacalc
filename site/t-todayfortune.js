TOOLS.push({id:"todayfortune",cat:"재미·운세",icon:"",name:"오늘의 운세",desc:"일진×일간 명리 풀이",render:function(el){
    ...
    function go(){
      ...
      var yongLine=yongHit?("오늘 일진 천간이 바로 그 "+SJ_EL[st.yong]+" — 용신이 들어오는 날입니다. 평소보다 판단이 선명하고 몸이 가볍습니다. 미뤄둔 결정은 오늘 내리세요."):
        yongClash?("오늘 일진 천간의 "+SJ_EL[todayEl]+" 기운이 용신 "+SJ_EL[st.yong]+josa(SJ_EL[st.yong],"를/을")+" 극하는 날입니다. 판단이 평소보다 무겁게 느껴질 수 있으니 큰 결정은 하루 미루는 편이 낫습니다."):
      ...
      '<div class="sj-sec"><h3>용신으로 보는 오늘</h3><p>당신의 사주는 '+(st.strong?"신강":"신약")+' — 억부법으로 '+SJ_EL[st.yong]+'('+EL_HAN.charAt(st.yong)+')'+josa(SJ_EL[st.yong],"가/이")+' 용신입니다. '+yongLine+' 용신 '+SJ_EL[st.yong]+josa(SJ_EL[st.yong],"는/은")+' '+SJ_YONG[SJ_EL[st.yong]].act+'에서 힘을 얻는 기운이라, 오늘 하루 중 이런 성격의 일에 시간을 쓰면 기운의 균형이 잡힙니다. 방향으로는 '+SJ_YONG[SJ_EL[st.yong]].dir+', 계절로는 '+SJ_YONG[SJ_EL[st.yong]].season+'의 기운이 나를 돕고, 일로는 '+SJ_YONG[SJ_EL[st.yong]].job+'과 결이 맞습니다.</p></div>'+
      '<div class="sj-sec"><h3>내 명식이 오늘 쓰는 힘</h3><p>'+sinLine+'</p></div>'+
      '<div class="sj-sec"><h3>오늘의 기운 — 십이운성 '+un+'</h3><p>'+conceptArt(ART_UN[un],un)+SJ_UN_DESC[un]+' 오늘 지지 '+SJ_B[tB]+'('+SJ_BH[tB]+')가 내 일간 '+SJ_S[me.d.s]+'에게 '+un+'의 자리라, 하루 동안 체감하는 기운의 결이 이렇습니다. 십이운성은 장생에서 시작해 제왕에서 정점을 찍고 절·태·양으로 돌아가는 열두 단계인데, '+un+josa(un,"는/은")+' 그중 '+(SJ_UN.indexOf(un)+1)+'번째 자리로 '+(SJ_UN.indexOf(un)<=4?"기운이 차오르는 상승 국면":SJ_UN.indexOf(un)<=8?"정점을 지나 힘을 거두는 하강 국면":"비우고 다시 준비하는 재생 국면")+'에 해당합니다.</p></div>'+
