var num=function(s){return Number(String(s).replace(/[^0-9.]/g,""))||0;};
  ...
  var UN_MOOD={
   ...
   "목욕":"감정이 풍부해지는 날이라 매력은 살지만, 기분 따라 결정하면 흔들리기 쉽습니다.",
   "관대":"의욕이 차오르는 날입니다. 자신감은 좋지만 서두르면 마무리가 거칠어집니다.",
   "건록":"발밑이 단단한 날입니다. 실속 있는 일부터 처리하면 하루가 알차게 쌓입니다.",
   "제왕":"기운이 정점이라 밀어붙이는 힘은 좋지만 과속을 조심할 날입니다.",
   "쇠":"속도를 줄이고 내실을 다지기 좋은 날입니다. 무리한 확장보다 정리가 어울립니다.",
   "병":"감수성이 깊어지는 날입니다. 몸의 신호에 민감해지니 컨디션부터 챙기세요.",
   "사":"생각이 안으로 파고드는 날이라, 혼자 집중하는 일에서 성과가 납니다.",
   "묘":"거두고 저장하는 날입니다. 새 판을 벌이기보다 모아둔 것을 지키는 쪽이 낫습니다.",
   "절":"흐름이 한 번 끊기고 다시 이어지는 날입니다. 변화가 와도 새 출발의 신호로 읽으세요.",
   "태":"아이디어가 씨앗처럼 맺히는 날입니다. 바로 실행보다 계획을 적어두기 좋습니다.",
   "양":"보호받으며 준비하는 날입니다. 서두르지 말고 힘을 기르는 데 쓰는 편이 좋습니다."};
  ...
  function birthDial(el,sel,onChange){
    ...
    function rebuildDays(){ // 2월 30일 같은 값이 남지 않도록 일 목록을 다시 만든다
      var max=daysIn(Y,M);if(D>max)D=max;
    ...
    function bind(col){
      col.addEventListener("scroll",function(){
        ...
        timer=setTimeout(function(){
          if(col.dataset.unit==="y"){Y=v;rebuildDays();}
          else if(col.dataset.unit==="m"){M=v;rebuildDays();}
          else D=v;
          commit();},120);});
      ...
      col.addEventListener("keydown",function(e){
        ...
        for(var i=0;i<items.length;i++)if(items[i]===cur){
