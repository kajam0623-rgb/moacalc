...
/* 새 페이지군이 배포돼도 되는지 판정한다.
   ...
   기준값 근거: (아래 Step 5 참고 — 이 listing의 기준값 67.8%/73.2%는 8개 표본에서 나온 틀린 값이고,
   전수 재측정 후 공통 크롬 제거 기준 평균 12.3% / 최대 47.9%로 바뀌었다) */
const fs = require("fs");
...
```
iljin-*  60개
  고유도   평균 12.3%   최대 47.9% (iljin-gyeongjin.html ↔ iljin-gyeongsul.html)
  크롬비중 76.4%   고유 본문 최소 iljin-imsul.html 399자 / 전체 3803자
  ✅ 통과
```

이 수치와 크게 다르면 **게이트 구현을 의심한다.** 제품은 그동안 안 바뀌었다.

> **이 Step의 앞 판은 틀렸었다.** 원래 "평균 약 67~68%, 최대 약 73%"를 기대값으로 적었는데, 그 최대값은
> 이전 세션이 **60개 중 8개만 표본으로** 재서 나온 값이었고 최악 쌍(경진·경술)이 표본에 없었다.
> 전수 1770쌍으로 재면 크롬 포함 최대는 **84.8%**다. 표본으로 기준값을 잡지 말 것.
>
> 더 크게는, **전 페이지 공통 3-gram이 문서당 평균의 76.4%**라 보일러플레이트를 포함해 재면
> 어떤 페이지군이든 68~85%에 몰려 게이트에 변별력이 없다. 그래서 `checkGroup`이 **모든 문서에 다 들어 있는
> 3-gram(= 공통 크롬)을 빼낸 뒤** 유사도를 재고, 임계값도 **평균 40% / 최대 70%**로 바뀌었다.
> 문서가 3개 미만이면 교집합이 과해지므로 크롬 제거를 건너뛴다.
>
> 위 Step 3의 코드 listing은 이 변경 **이전** 판이다. 실제 구현은 커밋된 `tools/gate.js`를 볼 것.
