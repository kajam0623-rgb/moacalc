...
> **완료 (2026-08-11).** T1~T4 전부 구현·배포·라이브 검증 완료. 아래는 원본 개발서이며 기록용으로 남긴다.
>
> | 지표 | 목표 | 라이브 실측 |
> |---|---|---|
> | 오늘의 운세 분량 | 1,400+ | **1,499자** |
> | 타로 분량 | 1,200+ | **최소 1,322 / 중앙 1,435자** (20회 측정 전부 통과) |
> | 페이지 수 | 81 | **82** (도구 57 + index + star 12 + zodiac 12) |
> | verify | 70+ | **89 통과 / 0 실패** |
>
> 측정 기준은 라이브 `innerText`(공백 제외)다. 개발 중 하네스가 공백 포함으로 재던 것을 라이브 실측에 맞춰 교정했다.
> 원고는 `content_star.js` · `content_zodiac.js`로 분리(3.5만자). build_site.js가 require한다.
> 신규 헬퍼 `josa(w,"와/과")` — 한글 받침 판정 조사 선택. verify HELPERS에 등록됨.
